...
`EasyAOBaker` Inspector の各項目を一覧でまとめます。日本語 UI 表示を基準に、対応する英語表記も併記します。

## 基本設定 (Basic)

常時表示される項目です。通常はここだけ触れば十分です。
...
| 日本語 UI 表示 | 英語表記 | 説明 |
| --- | --- | --- |
| 1st Shadow Scale / Offset | 1st Shadow Scale / Offset | 1 影レイヤーへの適用スケール・オフセット |
| 2nd Shadow Scale / Offset | 2nd Shadow Scale / Offset | 2 影レイヤーへの適用スケール・オフセット |
| 3rd Shadow Scale / Offset | 3rd Shadow Scale / Offset | 3 影レイヤーへの適用スケール・オフセット |
...
| 日本語 UI 表示 | 英語表記 | 説明 |
| --- | --- | --- |
| ベイクモード | Bake Mode | `RayCast` (正確・デフォルト) / `SSAO` (高速) |
...
:::note
`テクスチャのみ` は手動ベイクボタン押下時の動作を切り替える項目です。アップロード (Build & Test) 時は常にマテリアルに陰影が適用されます。
:::
...
陰影計算から除外したいメッシュに追加するマーカーコンポーネントです。設定項目はありません。追加するだけで、そのメッシュ (と配下のメッシュ) は他メッシュへの陰影計算に影響しなくなります。
