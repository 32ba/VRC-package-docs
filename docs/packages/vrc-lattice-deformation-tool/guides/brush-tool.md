...
<Icon name="brush" alt="ブラシツール" /> ブラシツールは、広い範囲をなぞりながら形を整える操作に向いています。
編集内容はブラシレイヤー `[B]` に記録されます。
...
標準 UI のブラシには 3 つのモードがあり、`Mesh Deformer` オーバーレイの `ブラシ変形` セクションで切り替えます。

| モード                |        アイコン        | 説明                                                                       |
| --------------------- | :--------------------: | -------------------------------------------------------------------------- |
| **法線** (Normal)     | <Icon name="normal" /> | メッシュの法線方向に沿って表面を押し出すか引き込む。最も基本的なモード     |
...
| 設定                                | 説明                                    |
| ----------------------------------- | --------------------------------------- |
| `ブラシ半径 (cm)`                   | ワールド空間での影響範囲 (0〜10 cm)      |
| `ブラシ強度 (%)`                    | 各ストロークで適用される強度 (0〜100%)   |
...
オーバーレイの <Icon name="mirror" /> `ミラーを有効化` をオンにすると、対称軸を挟んだ反対側にもブラシが適用されます。
`ミラー軸` では X、Y、Z のいずれかを選択します。
