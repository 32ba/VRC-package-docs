...
<Icon name="vertex-select" alt="頂点選択ツール" /> ブラシでは、選んだ頂点を同じ量だけ動かす操作が難しい場合があります。
頂点選択ツールを使うと、ブラシレイヤーの頂点を直接選び、移動、回転、スケールで正確に変形できます。
...
選択した頂点を 3 つのモードで変形できます。
キーボードショートカットで素早く切り替えられます。

| モード       |        アイコン        | キー | 説明                                     |
| ------------ | :--------------------: | ---- | ---------------------------------------- |
| **移動**     |  <Icon name="move" />  | W    | 選択した頂点をハンドルで移動             |
| **回転**     | <Icon name="rotate" /> | E    | 選択した頂点をピボットを中心に回転       |
| **スケール** | <Icon name="scale" />  | R    | 選択した頂点をピボットを中心に拡大または縮小 |

### <Icon name="pivot" /> ピボット

回転とスケールの基準点である**ピボット**を切り替えられます。
...
`プロポーショナル半径 (cm)` を `0` より大きくすると、選択した頂点の周囲も減衰カーブに応じて移動します。
少数の頂点を動かしながら、周囲へ滑らかに変形を広げられます。

{/* プロポーショナル半径を 0 より大きくした状態のスクリーンショット。緑色の影響半径が表示されている様子 */}
![](../../../../static/img/packages/vrc-lattice-deformation-tool/vertex-proportional.webp)

| 設定                         | 説明                                                            |
| ---------------------------- | --------------------------------------------------------------- |
| `プロポーショナル半径 (cm)` | 影響が周囲に広がる範囲。`0` で無効、最大 10 cm                 |
