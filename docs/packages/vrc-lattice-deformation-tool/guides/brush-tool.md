...
{/* TODO: ブラシツール起動後の Scene ビューのスクリーンショット。ブラシカーソル (円形) とオーバーレイパネルが見える状態 */}
...
| **マスク** (Mask) | | 変形させたくない領域を保護。塗った頂点は他のモードで影響を受けない |

{/* TODO: 法線モードで押し出し操作中のスクリーンショット。ブラシで膨らませている様子 */}

### マスクモードの詳細

- 塗った部分: **赤色**で表示 (保護)
- 塗っていない部分: **緑色**で表示 (編集可能)
- <Icon name="invert" /> `ブラシ反転` でマスクの消去もできます
- <Icon name="clear" /> `マスクをクリア` ボタンで全マスクをリセット

{/* TODO: マスクモードで一部の頂点がマスクされた状態のスクリーンショット。赤 (保護) と緑 (編集可能) の色分けが見える状態 */}
...
{/* TODO: 変位ヒートマップが有効な状態のスクリーンショット。レインボーカラーで変形量が可視化されている様子 */}
